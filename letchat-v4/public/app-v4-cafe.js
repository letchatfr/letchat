...
function updateRoomFeature() {
  ...
  if (!panel) return;
  if (currentPrivate || !["cafe", "amateurs", "webcam", "creatifs", "entraide"].includes(currentRoom)) {
  ...
  panel.innerHTML = `<div class="room-feature-title webcam-lobby-title"><div><b>▣ Membres disponibles en Webcam</b><span>${available.length ? `${available.length} membre${available.length > 1 ? "s" : ""} disponible${available.length > 1 ? "s" : ""}` : "Aucun autre membre disponible pour le moment"}</span></div><button id="testWebcamButton" type="button">Tester ma caméra</button></div>${available.length ? `<div class="webcam-members">${available.map((person) => `<article><span class="webcam-avatar">${person.photo ? `<img src="${safe(person.photo)}" alt="">` : safe(initials(person.name))}</span><strong>${safe(person.name)}</strong><small>Disponible maintenant</small><button type="button" data-webcam-message="${safe(person.id)}" data-webcam-name="${safe(person.name)}">Écrire</button><button type="button" class="webcam-call-primary" data-webcam-call="${safe(person.socketId || "")}" data-webcam-name="${safe(person.name)}" ${person.socketId ? "" : "disabled"}>Appeler</button></article>`).join("")}</div>` : `<div class="webcam-empty"><b>Personne n’est disponible actuellement</b><span>Revenez dans quelques minutes ou écrivez à un contact.</span></div>`}`;
...
function renderCafeOverview(panel) {
  const topics = ["Votre humeur du jour ?", "Que faites-vous ce week-end ?", "Quel film conseillez-vous ?", "Présentez votre ville"];
  ...
  panel.innerHTML = `<div class="cafe-overview"><div class="cafe-topics"><b>☀ Sujets du moment</b><div>${topics.map(topic => `<button type="button" data-cafe-topic="${safe(topic)}">${safe(topic)}</button>`).join("")}</div></div><div class="cafe-active"><b>Membres actifs</b><div>${active.length ? active.map(person => `<button type="button" data-cafe-profile="${safe(person.id)}" data-cafe-name="${safe(person.name)}" title="${safe(person.name)}">${person.photo ? `<img src="${safe(person.photo)}" alt="">` : safe(initials(person.name))}</button>`).join("") : '<span>Aucun autre membre pour le moment</span>'}</div></div>${pinned.length ? `<div class="cafe-pinned"><b>📌 Messages épinglés</b>${pinned.map(message => `<button type="button" data-pinned-message="${safe(message.id)}"><strong>${safe(message.author)}</strong><span>${safe(message.body || "Média partagé")}</span></button>`).join("")}</div>` : ""}</div>`;
  panel.querySelectorAll("[data-cafe-topic]").forEach(button => button.onclick = () => {
    $("#input").value = button.dataset.cafeTopic;
    $("#input").focus();
  });
  panel.querySelectorAll("[data-cafe-profile]").forEach(button => button.onclick = () => showPublicProfile(button.dataset.cafeProfile, button.dataset.cafeName));
...
function renderMeetingProfiles(panel) {
  ...
  const cities = [...new Set(visiblePeople.map(person => person.location?.city).filter(Boolean))].sort((a, b) => a.localeCompare(b, "fr"));
  panel.innerHTML = `<div class="meeting-panel"><div class="meeting-heading"><div><b>✦ Découvrir des profils</b><span>Choisissez les personnes avec qui vous souhaitez discuter.</span></div><div class="meeting-filters"><select id="meetingGender" aria-label="Filtrer par genre"><option value="all">Tous les profils</option><option value="female">Femmes</option><option value="male">Hommes</option><option value="neutral">Autres profils</option></select><select id="meetingCity" aria-label="Filtrer par ville"><option value="all">Toutes les villes</option>${cities.map(city => `<option value="${safe(city)}">${safe(city)}</option>`).join("")}</select></div></div><div id="meetingProfiles" class="meeting-profiles"></div></div>`;
  const draw = () => {
    const gender = $("#meetingGender").value, city = $("#meetingCity").value;
    const filtered = visiblePeople.filter(person => (gender === "all" || (person.gender || "neutral") === gender) && (city === "all" || person.location?.city === city));
    $("#meetingProfiles").innerHTML = filtered.length ? filtered.map(person => `<article class="meeting-profile"><span class="meeting-avatar">${person.photo ? `<img src="${safe(person.photo)}" alt="">` : safe(initials(person.name))}</span><div><strong>${safe(person.name)}${person.verified ? '<span class="verified-badge">✓</span>' : ""}</strong><small>${safe(person.location?.city || "Ville masquée")} · ${person.availability === "available" ? "Disponible" : person.availability === "busy" ? "Occupé" : "Absent"}</small><p>${safe(person.bio || "Aucune description pour le moment.")}</p></div><div class="meeting-actions"><button type="button" data-meeting-profile="${safe(person.id)}" data-meeting-name="${safe(person.name)}">Voir le profil</button><button type="button" data-meeting-message="${safe(person.id)}" data-meeting-name="${safe(person.name)}">Écrire</button><button type="button" class="meeting-call" data-meeting-call="${safe(person.socketId || "")}" data-meeting-name="${safe(person.name)}" ${person.socketId ? "" : "disabled"}>Appeler</button></div></article>`).join("") : `<div class="meeting-empty">Aucun profil ne correspond à ces filtres.</div>`;
